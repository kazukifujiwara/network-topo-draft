...
| D15 | **MCP Apps support (SEP-1865)**: the MCP server's render tool declares an interactive canvas as a `ui://` resource (`_meta.ui.resourceUri` → `ui://topodraft/canvas.html`, mimeType `text/html;profile=mcp-app`); the widget is the webview-ui canvas re-bootstrapped over the ext-apps `App` bridge through the same `AppHost` seam the VSCode webview uses — the renderer is never MCP-Apps-aware. Phase 1 is read-only: canvas edits are dropped behind a marked seam; interactive edits via `tools/call` on the existing edit tools are phase 2 | One renderer serves three hosts (VSCode webview, exported SVG, MCP Apps iframe) with zero forks. Pinned at adoption: spec revision **2026-01-26**, `@modelcontextprotocol/ext-apps` **1.7.4** |
...
| 2026-07-11 | **MCP Apps implementation shape (D15, v0.6.0)**: `packages/app-view` bundles the canvas + CSS into ONE self-contained HTML (build fails on remote references); the EXISTING `render_svg` is extended (no separate tool) — SVG text stays in `content` as the fallback, the canonical topology + view options travel in `structuredContent`, gated PER CLIENT on `capabilities.extensions["io.modelcontextprotocol/ui"]` so non-capable clients stay byte-identical to v0.5.0 (golden-verified) | Fallback for free, one discoverable render entry point, model-context cost unchanged |
| 2026-07-11 | **Correctness bar for MCP Apps work = protocol-valid + rendering in the ext-apps `basic-host`**; real-host rendering is best-effort (known host issues can leave the iframe unmounted while the protocol succeeds). basic-host gotchas: copy it OUT of the ext-apps clone before `npm install` (the workspace root's prepare script traps it) and add `@types/cors` + `cross-env`; the dev Streamable-HTTP harness (`packages/mcp/dev/serve-http.mjs`, never shipped) bridges our stdio server to it | basic-host is HTTP-only; keeps verification reproducible |
| 2026-07-11 | **Widget host-context policy**: pure theme changes are ignored (the canvas ships the fixed dark palette); layout-affecting changes (display mode, safe area, container resize) debounce into one re-fit; `ui/resource-teardown` releases the bridge's timers/listeners | An unknown theme value must never break the widget; light theme is a demand-driven follow-up |
