/**
 * Node glyphs (24×24 stroke-icon markup), ported verbatim from the frozen
 * v7 reference. Shared by the webview canvas and the SVG export generator;
 * colors are applied by each consumer (CSS custom properties in the
 * webview, concrete hex values in the generator).
 */
import type { IconKey } from './model';

/** Glyph keys: the core role classification plus the segment-node glyph. */
